<p align="right">
  <a href="README.md">🇬🇧 English</a>
</p>

<div align="center">
  <a href="https://github.com/FANDCS/Callen_App">
    <img src="/assets/icons/phonelefter_trans.png" height="100" alt="Callen Logo">
  </a>
  <h1>Callen</h1>
  Το συγκεκριμένο app παρέχεται, ενημερώνεται και ανήκει στον οργανισμό <b>FANDCS</b>.
</div>

## Υποστηριζόμενες Πλατφόρμες
* **Android:** 7+
* **Windows:** 10, 11
* **Linux:** Σε όλες τις διανομές (ως AppImage)

## Καταστήματα - Προσεχώς
### Android
- F-droid
- Uptodown
- ApkPure

### Windows
- Uptodown
- GitHub

### Linux
- Flathub
- GitHub

## Συντελεστές Έργου
- 🖥️ [Λευθέρης Τρόμπακας](https://www.github.com/AndroidCreator5)
- 🖌️ [Alex_632gr](https://www.instagram.com/alex632gr_/)
- 🦾 Claude Sonnet 5 (*Anthropic βάλε μεγαλύτερο free plan*)

## Πολιτική Απορρήτου & Όροι Χρήσης
[Πολιτική Απορρήτου & Όροι Χρήσης στο GitHub](https://raw.githubusercontent.com/FANDCS/main/refs/heads/main/Privacy_Policy_and_Terms_of_Use.md)

#### Τρέχουσα Έκδοση
`1.1 Beta Release`
